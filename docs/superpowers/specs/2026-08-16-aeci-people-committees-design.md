...
| D10 | **Identity numbers carry no year; transaction numbers do** | A member number is permanent. `EARF-2026-001` is filed and reported per year. **Amended 2026-08-24:** member numbers are no longer sequential either — see D19 |
| D19 | **Member numbers are random, with a check digit** (amended 2026-08-24) | A sequential `MEM-0001` tells anyone holding it roughly how many members the church has and who joined before whom. Acceptable on an internal form; not acceptable once the number is printed on an ID card a member carries (§7.5). Now `MEM-` plus six random digits and a Damm check digit, so a mistyped number is rejected rather than silently matching a different member. Register order comes from `date_became_member`, which is the date that actually matters |
